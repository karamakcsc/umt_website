frappe.pages['umt_test_drive'].on_page_load = function(wrapper) {
...
.cars-dropdown-options div {
    padding: 8px;
    cursor: pointer;
}

.cars-dropdown-options div:hover {
    ...
    <ul>
		...
		<section class="appointment" id="appointment">
			<div class="container">
				<h2>Appointment for Test Drive</h2>
    ...
    <div style="display: flex; gap: 1rem; margin-top: 1rem;">
        <div style="flex: 1;">
            ...
            <select id="cars-dropdown-options" name="cars-dropdown-options" required>
                <option value="0000">Select a car...</option>
            </select>
        </div>
        <div style="flex: 1;">
	...
	document.getElementById('bookAppointment').addEventListener('click', function() {
		window.location.href = 'umt_book_appointment'; 
	});

	document.getElementById('inquiry').addEventListener('click', function() {
		window.location.href = 'umt_inquiry'; 
	});

	document.getElementById('testDrive').addEventListener('click', function() {
		window.location.href = 'umt_test_drive'; 
	});

	document.getElementById('btnSubmit').addEventListener('click', submitToErpnext);

    const dropdownOptions = document.getElementById('cars-dropdown-options'); // Dropdown options container
    ...
    // document.addEventListener('DOMContentLoaded', () => {
        dropdownOptions.addEventListener('change', function () {
            ...
            if (selectedValue === '0000') {
                ...
                image.alt = 'Selected car image';
            }
        });

    frappe.call({
        method: "umt_website.umt_website.functions.umt.get_car_models",
        args: {
            doctype: "Item",
            fields: ["item_name", "image"],
            filters: {
                item_group: "Car Models"
            },
	...
	function submitToErpnext() {
        ...
        let selectedDropdown = document.getElementById('cars-dropdown-options');
        let selectedText = selectedDropdown.options[selectedDropdown.selectedIndex].text;
		...
		if (!fname || !lname || !email || !phone || !model) {
		...
		frappe.call({
			method: 'umt_website.umt_website.functions.umt.test_drive',
			args: { data: formData },
			callback: function(response) {
				if (response.message) {
					frappe.msgprint('Test Drive Request Sent Successfully!');
					document.querySelector('form').reset();
				} else {
					frappe.msgprint('Failed to create Test Drive Request. Please try again.');
				}
			}
		});
	}
